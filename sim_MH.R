...
n1 <- a[3]
kfold <- 3
B <- 0.05 #this is the constant C in the maintext
...
path.image <- paste0(path.save,'/n0=',n0,'B=',B,'Simu4.RData')
path.out <- paste0(path.save,'/n0=',n0,'B=',B,'simu4.out')
save.image(file=path.image)


path.graph <- paste0(path.save,'/n0=',n0,'B=',B,'biased1.pdf')
...
path.graph <- paste0(path.save,'/n0=',n0,'B=',B,'biased2.pdf')
pdf(file=path.graph, width=9, height=9)
op <- par(mfcol=c(1, 1), mai=c(0.6,0.5,0.6,0.4), 
          oma=c(2,2,2,2), cex=1)

# Simulation for Scenario II with unbiased summary statistics
...
simuOne <- function(n,n0,n1, kfold){
  ...
  ## oracle estimator (in this case, oracle estimator is the same as data-fused efficient estimator)
  result_ORC <- esti.EFF(x_inter, y_inter,tildabeta, V,n1)
...
path.image <- paste0(path.save,'/Simu3.RData')
path.out <- paste0(path.save,'/simu3.out')
save.image(file=path.image)


path.graph <- paste0(path.save,'/unbiased1.pdf')
...
path.graph <- paste0(path.save,'/unbiased2.pdf')
pdf(file=path.graph, width=9, height=9)
op <- par(mfcol=c(1, 1), mai=c(0.6,0.5,0.6,0.4), 
          oma=c(2,2,2,2), cex=1)
...
out <- round(cbind(sqrt(MSE[seq(1, 10, 2)]), c(ASE_INT[1], ASE_ORC[1], ASE_ADF[1], ASE_EFF[1], ASE_GIM[1]), 
                   c(cvp_INT[1], cvp_ORC[1], cvp_ADF[1], cvp_EFF[1], cvp_GIM[1]),
                   sqrt(MSE[seq(2, 10, 2)]), c(ASE_INT[2], ASE_ORC[2], ASE_ADF[2], ASE_EFF[2], ASE_GIM[2]), 
                   c(cvp_INT[2], cvp_ORC[2], cvp_ADF[2], cvp_EFF[2], cvp_GIM[2])) * 100, 2)

rownames(out) <- c()
write.csv(out, file = "res.tmp.csv", row.names = F)
